...
def plot_similarity_heatmap(df: pd.DataFrame) -> None:
    ...
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(
            title=dict(text="Similarity", font=dict(color=COLOR_TEXT)),
...
def plot_3d_document_space(docs: list, labels: list = None) -> None:
    ...
    colors = [
        f"hsl({int(240 + i * (120 / max(len(docs), 1)))}, 80%, 60%)"
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(
                ...
                gridcolor="#2D1B6E",
                showbackground=True,
                title=dict(text="PC1", font=dict(color=COLOR_MUTED)),
            ...
            yaxis=dict(
                ...
                gridcolor="#2D1B6E",
                showbackground=True,
                title=dict(text="PC2", font=dict(color=COLOR_MUTED)),
                tickfont=dict(color=COLOR_MUTED),
            ),
            zaxis=dict(
                backgroundcolor="#0B0618",
                gridcolor="#2D1B6E",
                showbackground=True,
                title=dict(text="PC3", font=dict(color=COLOR_MUTED)),
                tickfont=dict(color=COLOR_MUTED),
            ),
            bgcolor="#0B0618",
        ),
...
def plot_sentiment_chart(sentiments: list) -> None:
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            gridcolor="rgba(255,255,255,0.05)", title="Confidence", tickformat=".0%"
        ),
        margin=dict(l=10, r=10, t=20, b=10),
        height=350,
        showlegend=False,
...
def plot_topic_radar(topics: list) -> None:
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                gridcolor="rgba(255,255,255,0.1)", tickfont=dict(color=COLOR_TEXT)
            ),
        ),
        paper_bgcolor="rgba(0,0,0,0)",
        font=dict(color=COLOR_TEXT, family="Inter"),
...
def show_step_progress(steps: list, current: int) -> None:
    ...
    st.markdown(
        f'<div class="workflow-container">{steps_html}</div>', unsafe_allow_html=True
    )
