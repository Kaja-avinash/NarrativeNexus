...
def clean_text(text: str) -> str:
    """Remove problematic characters to prevent PDF encoding issues."""
    if not text:
        return ""
...
def generate_pdf(
    ...
    pdf.cell(
        0,
        8,
        f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')} | Version {APP_VERSION}",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    if file_names:
        pdf.cell(
            0,
            8,
            f"Documents: {', '.join([clean_text(f) for f in file_names[:5]])}",
            new_x="LMARGIN",
            new_y="NEXT",
        )
...
def generate_json_report(
    ...
    report = {
        ...
        "summary": summary,
        "topics": [
            {"topic_id": i + 1, "keywords": topic} for i, topic in enumerate(topics)
        ],
        "sentiments": [
            {
                "document": f"doc_{i + 1}",
